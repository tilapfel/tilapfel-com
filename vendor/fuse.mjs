...
const FUSE_MATCH_TOKEN_SEARCH_UNSUPPORTED =
...
var FuseIndex = class {
  ...
  create() {
    ...
    if (isString(this.docs[0]))
      ...
      for (let i = 0; i < len; i++) this.records[recordCount++] = this._createObjectRecord(this.docs[i], i);
  ...
  removeAt(idx) {
    ...
    for (let i = 0, len = this.records.length; i < len; i += 1)
      if (this.records[i].i > idx) this.records[i].i -= 1;
  }
  removeAll(indices) {
